...
export function ControlledCheckbox(props) {
    ...
    return (
        <label>{props.name}<input type="checkbox" onChange={props.onChange} checked={props.value} /></label>
...
export function CheckboxListWithState(props) {
    ...
    const itemsList= items.map((item)=> <li>{item[0]}<ControlledCheckbox value={item[1]} onChange={()=>setState({...state,[item[0]]:!item[1]})}/> </li>)
